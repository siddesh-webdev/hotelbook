...
function send_mail($email,$token,$type)
{
    if($type == "email_confirmation"){
     ...
     try {
         ...
         $mail->Username   = 'interstellar259@gmail.com';                     //SMTP username
         $mail->Password   = 'sjlzojbllhexfygg';                               //SMTP password
        ...
        else
        {
           ...
           if(!send_mail($data['email'],$token,'account_recovery'))
            ...
            else
            {
             ...
             $date = date("Y-m-d");
             $query= mysqli_query($cn,"UPDATE `user_cred` SET `token`='$token',`t_expire`='$date' WHERE `id`='$u_fetch[id]'");
