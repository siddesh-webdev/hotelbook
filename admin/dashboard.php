...
$is_shutdown =mysqli_fetch_assoc(mysqli_query($cn,"SELECT `shutdown` FROM `settings`"));

$current_booking = mysqli_fetch_assoc(mysqli_query($cn,"SELECT 
      COUNT(CASE WHEN booking_status='booked' AND arrival=0 THEN 1 END) AS `new_bookings`,
      COUNT(CASE WHEN booking_status='cancelled' AND refund=0 THEN 1 END) AS `refund_bookings`
      FROM `booking_order`"));

$unread_queries = mysqli_fetch_assoc(mysqli_query($cn,"SELECT COUNT(id) AS `count` FROM `user_queries` WHERE `seen`=0"));

$unread_reviews = mysqli_fetch_assoc(mysqli_query($cn,"SELECT COUNT(id) AS `count` FROM `rating_review` WHERE `seen`=0"));

$current_users = mysqli_fetch_assoc(mysqli_query($cn,"SELECT
      COUNT(id) AS `total`,
