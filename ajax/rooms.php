<?php
    ...
    {
      // available data decode
      $chk_avail =json_decode($_GET['chk_avail'],true);

        //checkin & checkout filter validtion
        ...
        //count number of rooms & out variable to store room cards
        $count_rooms = 0;
        $output="";
        
        //fetching setting tables to check shutdown is on
        $setting_q= "SELECT * FROM `settings` WHERE `id`=1";
        $setting_r = mysqli_fetch_assoc(mysqli_query($cn,$setting_q));
        ...
        while($room_data =mysqli_fetch_assoc($room_res))
        {
         ...
         if($chk_avail['checkin']!='' && $chk_avail['checkout']!='')
         {
            ...
            $fac_q= mysqli_query($cn,"SELECT f.name, f.id FROM `facilities` f 
